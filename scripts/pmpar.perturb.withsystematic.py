...
delta_ra = 1 #scale of perturbations in the RA
delta_dec = -1 #scale of perturbations in the Dec 

sigma_ra = 0.7 #scale of systematic error in RA
sigma_dec = 0.7 #scale of systematic error in Dec
...
if Z == [1]:
    ...
    SystemicRA = RNG.normal(0,sigma_dec) #systemic error for RA ["self".normal("mean", standard deviation)]
    SystemicDec = RNG.normal(0,sigma_dec) #systemic error for Dec
    #if we want a uniform distribution, use: RNG.uniform(lower bound, upper bound)
    
    obs1.perturbposition(rg1*delta_ra, rg2*delta_dec)#perturbs the position by the random offset 
    obs1.perturbposition(SystemicRA, SystemicDec) #adding in the systemic error (from a normal/gaussian distribution) 
    newcoords.append(obs1.to_string())#adds the perturbed positions into the observations array 
    
    
else: 
    RNG = Generator(PCG64(1)) #starts up the random number generator
    SystemicRA = RNG.normal(0,sigma_ra)#systemic error for RA ["self".normal("mean", standard deviation)]
    SystemicDec = RNG.normal(0,sigma_dec) #systemic error for Dec
...
for i in range(32):
    ...
    if Z == [1]:
        ...
        SystemicRA = RNG.normal(0,sigma_dec) 
        SystemicDec = RNG.normal(0,sigma_dec) 
        #if we want a uniform distribution, use: RNG.uniform(lower bound, upper bound)
        
        obs1.perturbposition(rg1*delta_ra, rg2*delta_dec)
        obs1.perturbposition(SystemicRA, SystemicDec)
...
perturbedout = open("new.pmpar.in" + ".withsystemic", "w") #writing and saving a new file named "new.pmpar.in.perturbed"
...
c1 = open("new.pmpar.in.withsystemic",'r')
c2 = c1.readlines()
c2
